...
#        ./crntb-adm.sh uninstall
        echo "Stopping club_ops.7x24hs.com service, begin killing ${PID}"
    ...
    if [ "$PID" != "" ] ; then
        echo "Starting club_ops.7x24hs.com service..."
        nohup python index.py -log_file_prefix=./log/club_ops.log &
        echo $! > pidfile    #record process id to file
        echo 'Startup club_ops.7x24hs.com service success!'
